...
# ----------------------------------------------------------------------
# Makefile generated from "Imake.tmpl" and </tmp/IIf.YjO795>
...
# operating system:  Linux 5.14.10-gentoo-x86_64 x86_64 [ELF] (5.14.10)
# libc:	(6.34.0)
# binutils:	(237)
...
SRCS = bezier.c blob.c color.c cursor.c fields.c frame.c gravity.c	kaleid.c life.c lightning.c lunch.c math.c password.c plasma.c	rose.c rotate.c starback.c swarm.c tag.c trippy.c waves.c xguts.c	mirrors.c fractals.c modes.c

OBJS=bezier.o blob.o color.o cursor.o fields.o frame.o gravity.o kaleid.o 	life.o lightning.o lunch.o math.o password.o plasma.o rose.o rotate.o 	starback.o swarm.o tag.o trippy.o waves.o xguts.o mirrors.o 	fractals.o modes.o
...
install:: xtacy
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTPGMFLAGS)  xtacy $(DESTDIR)$(BINDIR)/xtacy

all:: xtacy.$(MANNEWSUFFIX)

xtacy.$(MANNEWSUFFIX): xtacy.$(MANSRCSUFFIX)
	$(RM) $@
	if test -z "$(PREPROCESSMANPAGES)" ; then \
	   cd `dirname xtacy` && \
	   $(LN) `basename xtacy.$(MANSRCSUFFIX)` `basename $@`; \
	else \
	      	     $(RAWCPP)  $(MANDEFS) $(EXTRAMANDEFS) \
	     < xtacy.$(MANSRCSUFFIX) | sed -e '/^#  *[0-9][0-9]*  *.*$$/d' 			-e '/^#line  *[0-9][0-9]*  *.*$$/d' 			-e '/^[ 	]*XCOMM$$/s/XCOMM/#/' 			-e '/^[ 	]*XCOMM[^a-zA-Z0-9_]/s/XCOMM/#/' 			-e '/^[ 	]*XHASH/s/XHASH/#/' 			-e '/\@\@$$/s/\@\@$$/\\/' >$@; \
	fi

cleandir::
	$(RM) xtacy.$(MANNEWSUFFIX)

install.man:: xtacy.$(MANNEWSUFFIX)
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTMANFLAGS) xtacy.$(MANNEWSUFFIX) $(DESTDIR)$(MANDIR)/xtacy.$(MANSUFFIX)
